...
public class EventosServices {
	...
	public Eventos guardarEventos(Eventos eve) {
	    ...
	    // Verificar si la categoría ya está asociada a un evento en la misma sede
	    boolean categoriaYaAsociada = eventosEnSede.stream()
	        .anyMatch(evento -> evento.getCategorias().getId().equals(eve.getCategorias().getId()));
	    if (categoriaYaAsociada) {
	        throw new IllegalArgumentException("La categoría ya está asociada a un evento en esta sede.");
	    }
